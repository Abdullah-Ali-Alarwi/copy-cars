...
export default function SliderYear({
  ...
  const setYearRange = useCarStore((state) => state.setYearRange);
  ...
  // تحديث نطاق السنة في store وتطبيق الفلتر
  useEffect(() => {
    setYearRange([minValue, maxValue]);
    applyFilters();
  }, [minValue, maxValue, setYearRange, applyFilters]);

  const handleMinChange = (value: number) => {
    if (value >= min && value <= maxValue) setMinValue(value);
  };

  const handleMaxChange = (value: number) => {
    if (value <= max && value >= minValue) setMaxValue(value);
  };
